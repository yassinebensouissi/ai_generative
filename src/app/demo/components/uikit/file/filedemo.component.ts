...
import { StartupCanada } from 'src/app/demo/api/startupcanada';
...
export class FileDemoComponent implements OnInit {
    startups: StartupCanada[] = [];
    sortOptions: SelectItem[] = [];
    sortOrder: number = 0;
    sortField: string = '';
    filteredStartups: StartupCanada[] = [];
    ...
    ngOnInit() {
        ...
        this.filteredDataService.getFilteredDataCanada().subscribe(data => {
            this.filteredStartups = data;
        });
    }

    loadStartups() {
        this.startupService.GetAllStartupsCanada().subscribe(data => {
            this.startups = data;
    ...
    loadRegions() {
        this.startupService.getAllRegionsCanada().subscribe(data => {
    ...
    loadSectors() {
        this.startupService.getAllSectorsCanada().subscribe(data => {
